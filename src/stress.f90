...
subroutine getstressder2D(num_nodes, num_elems, nnz, &
     E, nodes, elems, data, rows, cols)
  ...
  ! Working
  double precision :: xy1(2), xy2(2), L
  integer :: ielem, k, index
  ...
  do ielem = 1, num_elems
     ...
     do k = 1, 2
        index = index + 1
        data(index) = E / L ** 2 * (xy1(k) - xy2(k))
        rows(index) = ielem
        cols(index) = 2 * (elems(ielem, 1) - 1) + k
        
        index = index + 1
        data(index) = E / L ** 2 * (xy2(k) - xy1(k))
        rows(index) = ielem
        cols(index) = 2 * (elems(ielem, 2) - 1) + k
     end do
  end do
