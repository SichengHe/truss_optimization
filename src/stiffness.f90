...
subroutine getresder2(num_nodes, num_elems, nnz, &
     E, nodes, elems, data, rows, cols, ind_aug)

  implicit none

  !f2py intent(in) num_nodes, num_elems, nnz, E, nodes, elems,
  !f2py intent(out) data, rows, cols, ind_aug
  !f2py depend(num_nodes) nodes
  !f2py depend(num_elems) elems
  !f2py depend(nnz) data, rows, cols, ind_aug
  ...
  ! Output
  double precision, intent(out) :: data(nnz)
  integer, intent(out) :: rows(nnz), cols(nnz), ind_aug(nnz)
  ...
  integer :: rows6x6(6, 6), cols6x6(6, 6)
  integer :: ones11(6, 6), ones12(6, 6), ones21(6, 6), ones22(6, 6)
  integer :: k1, k2, ielem, index, ind1, ind2
  ...
  do ielem = 1, num_elems
     ...
     do k1 = 1, 6
        do k2 = 1, 6
           ...
           data(index) = data(index) + data6x6(k1, k2)
           rows(index) = rows(index) + rows6x6(k1, k2) + 1
           ind_aug(index) = ind_aug(index) + cols6x6(k1, k2) + 1

           rows(index) = rows(index) + ones11(k1, k2) * ind1
           ind_aug(index) = ind_aug(index) + ones11(k1, k2) * ind1

           rows(index) = rows(index) + ones12(k1, k2) * ind1
           ind_aug(index) = ind_aug(index) + ones12(k1, k2) * ind2

           rows(index) = rows(index) + ones21(k1, k2) * ind2
           ind_aug(index) = ind_aug(index) + ones21(k1, k2) * ind1

           rows(index) = rows(index) + ones22(k1, k2) * ind2
           ind_aug(index) = ind_aug(index) + ones22(k1, k2) * ind2
