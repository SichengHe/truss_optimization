...
def read_files(u1, u2, geom_file, upp_file, low_file, results_file, factor):
    ...
    #print("num_chord,num_span",num_chord,num_span)
...
def getLength(nodes,elements):
    ...
    """get the length of truss"""
    n_elem = elements.shape[0]
    elem_len = numpy.zeros(n_elem)
    ...
    for i in xrange(n_elem):
        ...
        dx = xyz2-xyz1
        elem_len[i] = numpy.sqrt(numpy.transpose(dx).dot(dx))

    return elem_len

def getkFactor(elements,cons):

    """get the k factor for the buckling constraints"""

    n_elements = elements.shape[0]
    n_cons = len(cons)

    # check element is one side fixed
    one_end_fixed = []
    ...
    for i in xrange(n_elements):
        ...
        whether_contrained = []
        for ind in loc_elem:
            whether_contrained.append(any([ind == cons[j] for j in xrange(n_cons)]))

        if any(whether_contrained):
            if (whether_contrained[0] and whether_contrained[1]):
                print("ERROR: element %d both ends fixed" %(i))
            else:
                one_end_fixed.append(True)
        else:
            one_end_fixed.append(False)
    # if one side fixed: k = 0.699; if both free k=1.0
    k_list = []
    for i in xrange(n_elements):
        if (one_end_fixed[i]):
            k_list.append(0.699)
